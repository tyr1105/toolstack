...
function CronParser() {
  ...
  const parseField = (field: string, _min: number, max: number): string => {
    if (field === '*') return 'Every value (0-' + max + ')';
    if (field.includes('/')) {
      const [base, step] = field.split('/');
      return 'Every ' + step + ' from ' + (base === '*' ? 'start' : base);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 items-center">
        <input value={expression} onChange={(e) => setExpression(e.target.value)} placeholder="Enter cron expression..." className="flex-1 bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2.5 text-sm text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-blue-500/50 font-mono" />
        <button onClick={parse} className="btn-primary">Parse</button>
      </div>
      <div className="flex flex-wrap gap-2">
        {examples.map(({ label, expr }) => (
          <button key={label} onClick={() => setExpression(expr)} className="text-xs px-3 py-1.5 rounded-lg bg-zinc-800 text-zinc-300 hover:bg-zinc-700">{label}</button>
        ))}
      </div>
