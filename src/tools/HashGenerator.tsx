...
function HashGenerator() {
  ...
  const generateHashes = async () => {
    ...
    try {
      ...
      // MD5不在Web Crypto API中，使用简单的纯JS实现
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button onClick={generateHashes} disabled={loading} className="btn-primary">
          {loading ? 'Generating...' : 'Generate Hashes'}
        </button>
        <button onClick={() => { setInput(''); setHashes({}); }} className="btn-secondary">Clear</button>
      </div>
      <div>
        <label className="text-sm text-zinc-400 mb-1 block">Input Text</label>
        <textarea
          value={input}
      ...
      {Object.keys(hashes).length > 0 && (
        <div className="space-y-2">
          {Object.entries(hashes).map(([algo, hash]) => (
            <div key={algo} className="bg-zinc-900/50 border border-zinc-800/50 rounded-lg p-3 flex items-center gap-3">
              <span className="text-xs font-medium text-zinc-400 w-16 shrink-0">{algo}</span>
              <code className="flex-1 text-sm text-green-400 font-mono break-all">{hash}</code>
