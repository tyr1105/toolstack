...
// 主布局组件 - 包含头部导航、搜索和内容区域
function Layout({ children, currentTool }: LayoutProps) {
      ...
      <header className="sticky top-0 z-50 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-zinc-800/50">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            {searchQuery && filteredTools.length === 0 && (
              <div className="absolute top-full mt-1 w-full bg-zinc-900 border border-zinc-800 rounded-lg shadow-xl p-4 text-center text-zinc-500 text-sm">
                No tools found
              </div>
            )}
          </div>
          ...
          {/* GitHub链接 */}
          <a
            href="https://github.com/tyr1105/toolstack"
            target="_blank"
            rel="noopener noreferrer"
            className="text-zinc-400 hover:text-white transition-colors text-sm hidden sm:flex items-center gap-1"
          >
